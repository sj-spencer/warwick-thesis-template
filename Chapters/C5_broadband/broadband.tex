...
One subtle thing is that broadband takes various functional forms, and the plasma might care about that.
...
\end{equation} and is verified through PIC simulations in the kinetic and fluid regimes. The left-hand side of Eq. \ref{eqn:maxGainCondition} represents the maximum chirp ($|\partial_t\omega_0(x,t)|$) of the laser, and the right-hand side gives an expression for the spatial detuning due to density inhomogeneity. Several simplifying assumptions are made by the authors which allow them to derive this expression. Most importantly for our work, they omit the contribution of the non-linear frequency shift in their calulcation of the velocity of the resonance point. 

A key result of this work is that the iSRS threshold is independent of the bandwidth or frequency modulation alone, rather it depends on their product. Figure \ref{fig:Wenreplication} shows results from our investigations into the 
...
\hline
\begin{tabular}[c]{@{}l@{}}Wavelength /\\ nm\end{tabular} & \begin{tabular}[c]{@{}l@{}}Intensity / \\ W$\text{cm}^{-2}$\end{tabular} & \begin{tabular}[c]{@{}l@{}}Bandwidth / \\ THz\end{tabular} & \begin{tabular}[c]{@{}l@{}}Num. beamlets \\ \end{tabular} 
& \begin{tabular}[c]{@{}l@{}}Modulation \\ frequency / THz \end{tabular}\\ \hline 
&&&\\[-1em]
193   & $4\times 10^{15}$   & 0,5,8,10  & 3,50 & n.a. \\ \hline
&&&\\[-1em]
351   & 3                   & 0,1,fromWen  & n.a. & fromWen \\ \hline
\end{tabular}

\caption{Table shows laser parameters for this investigation}
\end{table}

\missingfigure{Plasma temperature and density profiles at t=start of spike from NRL}

$\kld$ is between 0.3 and 0.45
